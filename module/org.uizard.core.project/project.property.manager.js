 ...
 **/
org.uizard.core.project.property.manager = function () {
	this.ini = null;
	this.treeView = null;
	this.tabView = null;
};

org.uizard.core.project.property.manager.prototype = {
	...
	createTabView: function (xml,label) {
		var self = this;
		if ($(xml).find("ITEM[label="+label+"]").length > 0){
			$(xml).find("ITEM[label="+label+"]").each(function(){
				if ($(this).find("TAB").length > 0) {
					self.tabView = new YAHOO.widget.TabView();
					$(this).find("TAB").each(function(){
						...
						self.tabView.addTab( new YAHOO.widget.Tab({ 
					...
					$("#propertyTabview").html("");
					self.tabView.appendTo("propertyTabview");
					$("#propertyTabview .yui-content")
					...
					$("#propertyTabview").html("<div class='yui-content'>"+content+"</div>");
					$("#propertyTabview .yui-content")
			...
			if ($(xml).find("ROOT[label="+label+"]").children("ITEM").length == 0){
				var content="";
				$(xml).find("ROOT[label="+label+"]").each(function(){
					if($(this).attr("url")){
						var url = $(this).attr("url");
	...
	iniParser: function () {
		var self=this;
		var url = "module/org.uizard.core.project/project.property.ini.parser.php";
		$.ajax({
			url: url,			
			type: "POST",
			async : false,
